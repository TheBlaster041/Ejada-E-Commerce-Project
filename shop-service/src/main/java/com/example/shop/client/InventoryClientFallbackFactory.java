...
public class InventoryClientFallbackFactory implements FallbackFactory<InventoryClient> {
    ...
    public InventoryClient create(Throwable cause) 
    {
        ...
        return new InventoryClient() 
        {
            ...
            @Override
            public Map<String, Object> remove(String sku,int quantity) 
            {
                throw createException(cause);
            }
        };
    ...
    private RuntimeException createException(Throwable cause)
    {
        FeignException feignException = findFeignException(cause);
        if (feignException != null) 
        {
            int status = feignException.status();
            String message = extractMessage(feignException.contentUTF8());
            return new InventoryServiceException(status,message);
        }
        return new InventoryServiceException(503,"Inventory service unavailable");
    }
